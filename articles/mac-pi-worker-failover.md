...
この構成は手軽です．自宅ルーターのポートを開けずに済み，Mac側の `localhost:3000` をそのまま公開できます．一方で，originはあくまで自宅のMacです．Macを再起動すれば落ちますし，スリープやネットワーク不調の影響も受けます．実際，Macを49.7日ほど起動し続けるとTCPまわりが壊れる問題にも遭遇していて，そのときの検証は別記事にまとめています．

https://zenn.dev/inazumimakoto/articles/mac-tcp-report
...
`inazu.me` が完全に静的なHTML 1枚だけなら，そもそも自宅Macで持つ必要はありません．Cloudflare PagesやGitHub Pagesに置けば済みます．あるいは，同じファイルを複数の場所に置いて冗長化するだけでもよいです．

ただ，今の `inazu.me` は「静的サイト」と言い切るには少し面倒な作りになっています．背景画像を大量に持っていて，時間帯ごとに画像を切り替えます．liquid glass風の見た目もあり，やかましい紙吹雪も降ります．Mac側のExpressは，背景画像一覧を `/api/backgrounds` で返すなど，単にHTMLを置くだけではない役割も持っています．

理想を言えば，同じものをもう1台でも動かして冗長化したいです．ただ，fallback先として用意しているのはRaspberry Pi Zero 2 Wです．今の `inazu.me` と同じ見た目，同じ画像群，同じ周辺機能をそのまま載せるには心もとないですし，運用も重くなります．

さらに，`chat.inazu.me` も同じMacにいます．こちらはMac上のローカルLLMと対話するWebアプリなので，Raspberry Pi Zero 2 Wで同じ性能の代替originを作るのは現実的ではありません．Pi側でできるとしても，せいぜい「今はMacが落ちています」といった固定応答です．

そのため，今回の方針は「全部を同じ品質で冗長化する」ではなく，「Macが落ちたら，性能や機能を落とした代替表示へ切り替える」です．普段はMacを正とし，障害時だけPiの簡素ページ，それもだめならWorker内蔵HTMLへ落とす．この意味で，冗長化というよりフェイルオーバーとして作る方が目的に合っていました．

## なぜ友人宅に置くのか

Raspberry Piを自宅に置く案もあります．しかし，自宅Macのfallbackを同じ家に置くと，自宅の停電や回線障害でまとめて落ちます．Macだけが再起動した，プロセスだけが死んだ，というケースには効きますが，家ごと落ちるケースには効きません．

フェイルオーバーを考えるなら，本来は電源，回線，場所を分けるのが基本です．理想を言えば，地理的にも十分離れた場所に置きたいところです．

今回は，知人の家にRaspberry Piを置く予定です．とはいえ，場所は隣町くらいです．大規模停電や広域の回線障害が起きたら普通に一緒に落ちる可能性があります．それはもうフェイルオーバーとは何か，という話になってきます．

ただ，少なくとも自宅のブレーカーが落ちた，自宅回線が死んだ，自宅Macが固まった，という範囲では，自宅とは別の電源・別の回線に逃げられます．個人サイトの実験としては，このくらいの距離感がちょうどよい落としどころでした．

また，遠くに置きすぎると物理メンテナンスがつらくなります．後述するTailscaleでSSHできるとはいえ，SDカードや電源，ケーブルまわりで何かあったときは現地作業が必要になります．隣町くらいなら，地理的には少し分けつつ，必要になれば見に行ける距離でもあります．
