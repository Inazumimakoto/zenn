...
もちろん，これらを全部どこかの静的ホスティングへ移すこともできます．しかし，今回やりたいのは「Macで動いている今の本番サイトを通常時はそのまま使い，Macが落ちたときだけ簡素な代替表示にする」ことでした．

つまり，同じものを常に複数台で動かす冗長化ではなく，普段はMacを正とし，障害時だけ別の表示へ切り替えるフェイルオーバーの方が目的に合っています．

`chat.inazu.me` も同じです．こちらはMac上のローカルLLMと対話するWebアプリなので，Raspberry Pi Zero 2 Wで同じ性能の代替originを作るのは現実的ではありません．Pi側でできるとしても，せいぜい「今はMacが落ちています」といった固定応答です．

そのため，今回の方針は「全部を同じ品質で冗長化する」のではなく，「Macが落ちたときに，落ちたことが分かる簡素な代替表示へ切り替える」です．これをやるには，Cloudflare Workerで入口を制御し，Mac，Pi，Worker内蔵HTMLの順に落としていく構成が扱いやすいと判断しました．
